...
const [active, setStatus]=useState<number>(0)
    ...
    const btnHandler=(i:number, e:React.MouseEvent<HTMLButtonElement, MouseEvent>)=>{
        console.log("e: ",e,"i:",  i , 'active=' ,  active);
        setStatus(i)
        console.log(   'isActive=' , active===i , active, i)
    }
    ...
    return (
        <div className={`${s.tabSwitcher} ${variant && s[variant]}  ${className}`} {...rest}>
            <h4>title</h4>
            {buttonsName.map(
                (b:string, i:number)=> <Button key={i} variant={buttonsVariant && buttonsVariant} className={` ${s.button}  ${active===i && s.btnActive }`}
